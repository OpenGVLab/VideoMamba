...
def create_dataset(dataset_type, config):
    ...
    if "swin" in vision_enc_name or "vit" in vision_enc_name or "videomamba" in vision_enc_name:
    ...
    if dataset_type in ["pt_train", "pt_ds_train", "ret_train"]:
        ...
        for m in train_media_types:
            ...
            if dataset_type == "pt_train":
                dataset_cls = SQLiteImgTxtRetTrainDataset if m == "image" else SQLiteVidTxtRetTrainDataset
    ...
    elif dataset_type in ["pt_eval", "pt_ds_eval", "ret_eval"]:
        test_datasets = []
        test_dataset_names = []
        ...
        for name, data_cfg in config.test_file.items():
            ...
            dataset_kwargs = dict(
                ...
                is_act_rec=False # use for action recognition
            )
            if media_type == "video":
