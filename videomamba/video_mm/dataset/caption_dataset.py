...
from dataset.text_prompt import kinetics_templates, kinetics_templates_plus, ssv2_templates
...
class ImgTxtRetEvalDataset(ImageVideoBaseDataset):
    ...
    def __init__(self, ann_file, transform, has_multi_vision_gt=False, is_act_rec=False, is_ssv2=False):
        super(ImgTxtRetEvalDataset, self).__init__()
        self.is_ssv2 = is_ssv2
    ...
    def build_data_act_rec(self):
        ...
        templates = kinetics_templates if not self.is_ssv2 else ssv2_templates
        text = list(set([e["caption"] for e in self.raw_anno_list]))
        text2label = {e: i for i, e in enumerate(text)}
        text = [[t.format(e) for t in templates] for e in text]
        ...
        self.txt2img = [[] for _ in range(len(text) // len(templates))]
...
class VidTxtRetEvalDataset(ImgTxtRetEvalDataset):
    ...
    def __init__(
            ...
            is_paragraph_retrieval=False, has_multi_vision_gt=False,
            trimmed30=False, is_act_rec=False, is_ssv2=False
    ):
        super(VidTxtRetEvalDataset, self).__init__(ann_file, transform, has_multi_vision_gt, is_act_rec, is_ssv2)
        self.num_frames = num_frames
        self.video_reader_type = video_reader_type
        self.video_reader = VIDEO_READER_FUNCS[video_reader_type]
        self.sample_type = sample_type
