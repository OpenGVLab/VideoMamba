...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.CrossEntropyLoss,
                    ...
                    device: torch.device, epoch: int, loss_scaler, amp_autocast, teacher_model, max_norm: float = 0,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.no_grad():
            with amp_autocast:
                _, feats_teaceher = teacher_model(samples)
         
        with amp_autocast:
            outputs, feats_student = model(samples)
            loss_cls = criterion(outputs, targets)
            loss_distill = (2 - 2 * (feats_student * feats_teaceher).sum(dim=-1)).mean()
